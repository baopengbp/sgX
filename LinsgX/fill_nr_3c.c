...
/*
 * Add GTOnr3c_fill_scr_s1, GTOnr3c_fill_scr_s2ij, GTOnr3c_scr_drv
 * for LinsgX by Peng Bao <baopeng@iccas.cn>
 */
...
void GTOnr3c_fill_scr_s1(int (*intor)(), double *out, double *buf,
                     int comp, int jobid,
                     int *shls_slice, int *ao_loc, CINTOpt *cintopt,
                     int *atm, int natm, int *bas, int nbas, double *env, 
                     int *vsh, int nvsh, int nvshi, int *ivx, int *vov, 
                     double *fg, double *gv)
{
        const int ish0 = shls_slice[0];
        const int ish1 = shls_slice[1];
        ...
        const int ksh1 = shls_slice[5];
        const int nksh = ksh1 - ksh0;
        const int ksh = jobid + ksh0;

        const size_t naoi = nvshi;
        const size_t naoj = nvshi;
        const size_t naok = ao_loc[ksh1] - ao_loc[ksh0];
        const int dims[] = {naoi, naoj, naok};
        const int k0 = ao_loc[ksh] - ao_loc[ksh0];

        out += naoi * naoj * k0;
        int ish, jsh, i0, j0, j, k;
        int shls[3] = {0, 0, ksh};

        for (jsh = 0; jsh < nvsh; jsh++) {
        for (ish = 0; ish < nvsh; ish++) {

                shls[0] = vsh[ish];
                shls[1] = vsh[jsh];
                i0 = ivx[ish];
                j0 = ivx[jsh];
                if (vov[ish+jsh*nvsh] == 1) {
                        (*intor)(out+j0*naoi+i0, dims, shls, atm, natm, bas, nbas, env,
                                 cintopt, buf);
                }
        } }
        for (j = 0; j < nvshi; j++) {
            for (k = 0; k < nvshi; k++) {
                gv[jobid*nvshi+j] += out[k*nvshi+j] * fg[jobid*nvshi+k];
            }
        }
}
                       ...
                       int *atm, int natm, int *bas, int nbas, double *env)
{
        ...
        const size_t nij = (i1-i0)* (i1-i0 + 1) / 2 ;
        ...
        for (jsh = jsh0; jsh < jsh1; jsh++) {
                ...
                //pout = out + (ip-i0) * (ip-i0 + 1) / 2 + jp-i0;
                pout = out + ip * (ip + 1) / 2 + jp;
                if (ip != jp) {
...
void GTOnr3c_fill_scr_s2ij(int (*intor)(), double *out, double *buf,
                       int comp, int jobid,
                       int *shls_slice, int *ao_loc, CINTOpt *cintopt,
                       int *atm, int natm, int *bas, int nbas, double *env, 
                       int *vsh, int nvsh, int nvshi, int *ivx, int *vov, 
                       double *fg, double *gv)
{
        ...
        //const size_t nij = i1 * (i1 + 1) / 2 - off;
        //const size_t nij = (i1-i0)* (i1-i0 + 1) / 2 ;
        const size_t nij = nvshi * (nvshi + 1) / 2 ;
        const size_t nijk = nij * naok;
        ...
        for (jsh = 0; jsh < nvsh; jsh++) {
                ...
                shls[0] = vsh[ish];
                shls[1] = vsh[jsh];
                di = ao_loc[vsh[ish]+1] - ao_loc[vsh[ish]];
                dj = ao_loc[vsh[jsh]+1] - ao_loc[vsh[jsh]];

                if (vov[jsh+ish*nvsh] == 1) {
                        (*intor)(buf, NULL, shls, atm, natm, bas, nbas, env, cintopt, cache);
                        if (ip != jp) {
                                for (j = 0; j < di; j++) {
                                for (k = 0; k < dj; k++) {
                                        gv[jobid*nvshi+ip+j] += buf[j+k*di] * fg[jobid*nvshi+jp+k];
                                        gv[jobid*nvshi+jp+k] += buf[j+k*di] * fg[jobid*nvshi+ip+j];
                                } }
                        } else {
                                for (j = 0; j < di; j++) {
                                for (k = 0; k < dj; k++) {
...
void GTOnr3c_scr_drv(int (*intor)(), void (*fill)(), double *eri, int comp,
                 int *shls_slice, int *ao_loc, CINTOpt *cintopt,
                 int *atm, int natm, int *bas, int nbas, double *env, 
                 int *vsh, int nvsh, int nvshi, int *ivx, int *vov, double *fg, double *gv)
{
        const int ish0 = shls_slice[0];
        const int ish1 = shls_slice[1];
        const int jsh0 = shls_slice[2];
...
#pragma omp parallel default(none) \
        shared(intor, fill, eri, comp, shls_slice, ao_loc, cintopt, \
               atm, natm, bas, nbas, env, vsh, nvsh, nvshi, ivx, vov, fg, gv)
{
        int jobid;
        double *buf = malloc(sizeof(double) * (di*di*di*comp + cache_size));
#pragma omp for nowait schedule(dynamic)
        for (jobid = 0; jobid < nksh; jobid++) {
                (*fill)(intor, eri, buf, comp, jobid, shls_slice, ao_loc,
                        cintopt, atm, natm, bas, nbas, env, 
                        vsh, nvsh, nvshi, ivx, vov, fg, gv);
        }
        free(buf);
}
