...
void SGXsetnr_direct_scf_blk(CVHFOpt *opt, int (*intor)(), CINTOpt *cintopt,
                         int *ao_loc, int *atm, int natm,
                         int *bas, int nbas, double *env, int *bvv, int nbvv)
{
        if (opt->q_cond) {
                free(opt->q_cond);
        }
        ...
        const int cache_size = GTOmax_cache_size(intor, shls_slice, 1,
                                                 atm, natm, bas, nbas, env);
#pragma omp parallel default(none) \
        shared(intor, q_cond, ao_loc, atm, natm, bas, nbas, env, bvv, nbvv)
{
        double qtmp, tmp;
        int ij, i, j, di, dj, ish, jsh;
        int shls[2];
...
#pragma omp for schedule(dynamic, 4)
        for (ij = 0; ij < nbas*(nbas+1)/2; ij++) {
                ...
                if  (sind < 0 || sjnd < 0){                                
                        q_cond[ish*nbas+jsh] = 0;
                        q_cond[jsh*nbas+ish] = 0;
                        continue;
                }
