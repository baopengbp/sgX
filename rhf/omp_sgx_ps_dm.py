...
'MKL_NUM_THREADS=16 OMP_NUM_THREADS=16 python omp_sgx_ps.py'
'''
...
# Kuv = Sum(Xug Avt Dkt Xkg)
    ...
    for i0, i1 in lib.prange(0, ngrids, blksize):
        ...
        gbn = bn.swapaxes(0,2)
        jg = numpy.dot(gbn.reshape(-1,nao*nao), dm.reshape(-1))
        xj = lib.einsum('gv,g->gv', wao_v[i0:i1], jg)
        sgj += lib.einsum('gu,gv->uv', wao_v[i0:i1], xj)
    ...
    snsgj = scipy.linalg.solve(sn, sgj)
    vj = numpy.matmul(ovlp, snsgj)
    return vj, vk
