...
'MKL_NUM_THREADS=16 OMP_NUM_THREADS=16 python omp_sgx_dfj_direct_ddm.py'
'''
...
def get_veff(mol, dm, dm_last=0, vhf_last=0, hermi=1):
    ...
    if grdchg == 0 :
        ...
        return vj - vk * .5
# from uhf.get_veff
def get_veffuhf(mol, dm, dm_last=0, vhf_last=0, hermi=1, vhfopt=None):
    dm = numpy.asarray(dm)
    nao = dm.shape[-1]
